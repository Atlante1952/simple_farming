...
if minetest.get_modpath("awards") then
	...
	awards.register_award("the_month_of_harvest_2",{
		...
		trigger = {
			type = "dig",
			node = "simple_farming:tomatoes_5",
			target = 500
		}
	})
	...
	awards.register_award("storing_crops",{
		...
		trigger = {
			type = "craft",
			item = "simple_farming:onion_crate",
			target = 50
		}
	})
	...
	awards.register_award("tomatoes_are_good",{
		...
		trigger = {
			type = "craft",
			item = "simple_farming:tomato_juice",
			target = 50
		}
	})
	...
	awards.register_award("gingerbread_man",{
		...
		trigger = {
			type = "craft",
			item = "simple_farming:gingerbread_man",
			target = 50
		}
	})
	...
	awards.register_award("candy_cane_block",{
		...
		trigger = {
			type = "place",
			item = "simple_farming:candy_cane_block",
			target = 200
		}
	})
	...
	awards.register_award("secure_a_bright_future",{
		...
		trigger = {
			type = "place",
			item = "simple_farming:seed_pepper",
			target = 250
		}
	})
end
